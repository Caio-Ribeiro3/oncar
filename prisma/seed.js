...
async function main() {
    const json = [{ "brand": "Seat", "models": ["Alhambra", "Altea", "Altea XL", "Arosa", "Cordoba", "Cordoba Vario", "Exeo", "Ibiza", "Ibiza ST", "Exeo ST", "Leon", "Leon ST", "Inca", "Mii", "Toledo"] },
    ...
    await Promise.all(json.map(({ brand, models }) => prisma.brand.create({
        data: {
            name: brand,
            models: {
                create: models.map(model => ({ name: model }))
            }
        }
    })))
}
